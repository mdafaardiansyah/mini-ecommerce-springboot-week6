...
├── README.md                # This file
├── actuator/
│   └── api-spec.md         # Actuator endpoints (health, metrics, info)
├── category.md             # Category API specification
...
**File:** [`actuator/api-spec.md`](actuator/api-spec.md)

**Overview:**
Spring Boot Actuator provides production-ready features to help you monitor and manage your application.

**Endpoints:**
- `GET /actuator/health` - Application health status (database, disk space, etc)
- `GET /actuator/info` - Application information (build, Git, Java, OS details)
- `GET /actuator/metrics` - List of available metrics
- `GET /actuator/metrics/{metric.name}` - Detailed metric information
- `GET /actuator/env` - Environment properties and configuration
- `GET /actuator/env/{property.name}` - Specific property value
...
**Actuator endpoints may return different formats depending on the endpoint.**
...
| 503  | SERVICE_UNAVAILABLE | Application unhealthy (Actuator health) |
...
```bash
# Simple health check
curl -f https://idp-week6.glanze.space/actuator/health || exit 1

# Detailed health check
curl https://idp-week6.glanze.space/actuator/health | jq '.'
```
...
```bash
# Basic health check
curl https://idp-week6.glanze.space/actuator/health

# Check specific components
curl https://idp-week6.glanze.space/actuator/health | jq '.components'
```
...
```bash
# JVM Memory Usage
curl https://idp-week6.glanze.space/actuator/metrics/jvm.memory.used

# Database Connections
curl https://idp-week6.glanze.space/actuator/metrics/hikaricp.connections.active

# HTTP Request Metrics
curl https://idp-week6.glanze.space/actuator/metrics/http.server.requests
```
...
```bash
# Application Info
curl https://idp-week6.glanze.space/actuator/info | jq '.'

# Get application version
curl https://idp-week6.glanze.space/actuator/info | jq '.build.version'

# Get Git commit
curl https://idp-week6.glanze.space/actuator/info | jq '.git.commit.id'
```
